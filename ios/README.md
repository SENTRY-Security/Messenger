...
### 2) App Clip
- `SentryMessengerClip` target（bundle id `red.sentry.app.SENTRY-Messenger.Clip`）。
...
- **AASA**：`web/functions/.well-known/apple-app-site-association.ts`（與 root 版）
  已加入 `appclips` 與 `applinks`，appID `DYAAM5G8JF.red.sentry.app.SENTRY-Messenger(.Clip)`。
...
> Bundle ID：App `red.sentry.app.SENTRY-Messenger`、Clip `…​.Clip`；Team `DYAAM5G8JF`
> （與線上 AASA 一致）。
